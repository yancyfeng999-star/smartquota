...
This document describes the pluggable theme architecture in SmartQuota.

## Overview

SmartQuota uses a **protocol-based theme system** that allows easy creation and registration of new themes. The design follows the same patterns used throughout the codebase (ISP, Protocol-Based DI).
...
SmartQuota can import `.itermcolors` files to automatically generate themes from terminal color schemes. Over 450 pre-made schemes are available at [iTerm2-Color-Schemes](https://github.com/mbadolato/iTerm2-Color-Schemes).
...
3. **Persist** — Schemes are saved as JSON in `~/.smartquota/themes/`
