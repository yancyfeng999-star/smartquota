...
struct ScriptProbeTests {
    ...
    func `probe runs without env vars when no config fields`() async throws {
        ...
        let command = capturedArgs.last ?? ""
        #expect(!command.contains("SMARTQUOTA_"))
    }
    ...
    @Test
    func `probe refuses scripts that escape the extension directory`() async throws {
        let executor = MockCLIExecutor()
        var capturedArgs: [String] = []
        given(executor)
            .execute(binary: .any, args: .any, input: .any, timeout: .any, workingDirectory: .any, autoResponses: .any)
            .willProduce { _, args, _, _, _, _ in
                capturedArgs = args
                return CLIResult(output: #"{"quotas":[]}"#, exitCode: 0)
            }
        ...
        _ = try? await probe.probe()
        let command = capturedArgs.last ?? ""
        // Must not invoke /usr/bin/true — path confined to extension dir
        #expect(!command.contains("/usr/bin/true"))
        #expect(command.contains("__denied__") || command.isEmpty || command.contains("/ext/"))
    }
}
