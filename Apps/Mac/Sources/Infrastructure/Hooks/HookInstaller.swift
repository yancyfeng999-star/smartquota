...
/// Installs and uninstalls 智额 hooks in ~/.claude/settings.json.
/// Hook commands use the __smartquota_hook function wrapper for identification.
public enum HookInstaller {
    /// The marker function name used to identify 智额 hooks
    static let hookMarker = AppIdentity.hookMarker
    ...
    static var hookCommand: String {
        let marker = AppIdentity.hookMarker
        let portFile = AppIdentity.hookPortFileName
        return """
        \(marker)() { PORT=$(cat "$HOME/.claude/\(portFile)" 2>/dev/null || echo \(HookConstants.defaultPort)); cat | curl -s -X POST "http://localhost:${PORT}/hook" -H 'Content-Type: application/json' -d @- > /dev/null 2>&1 & }; \(marker)
        """
    }
    ...
    public static func install() throws {
        ...
        for event in hookEvents {
            ...
            // Remove any existing 智额 matcher entries for this event
            matcherEntries.removeAll { entry in
                containsAppHook(in: entry)
            }
    ...
    /// Uninstalls 智额 hooks from the Claude settings file.
    ...
    /// Detects whether 智额 hooks are currently installed.
    ...
    /// Checks if a matcher entry contains a 智额 hook command.
    private static func containsAppHook(in matcherEntry: [String: Any]) -> Bool {
        guard let innerHooks = matcherEntry["hooks"] as? [[String: Any]] else { return false }
        return innerHooks.contains { hook in
            guard let command = hook["command"] as? String else { return false }
            return command.contains(AppIdentity.hookMarker)
        }
    }
