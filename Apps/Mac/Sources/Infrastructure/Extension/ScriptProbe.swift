...
public final class ScriptProbe: UsageProbe, @unchecked Sendable {
    ...
    /// Resolve script path strictly under `extensionDir`.
    /// Paths that escape the extension folder (absolute outside, or `..`) are rejected.
    private func resolveScriptPath() -> String {
        let extRoot = extensionDir.standardizedFileURL

        let candidate: URL
        if scriptPath.hasPrefix("/") {
            candidate = URL(fileURLWithPath: scriptPath).standardizedFileURL
        } else {
            candidate = extRoot.appending(path: scriptPath).standardizedFileURL
        }

        let rootPath = extRoot.path
        let candPath = candidate.path
        let prefix = rootPath.hasSuffix("/") ? rootPath : rootPath + "/"
        let isInside = candPath == rootPath || candPath.hasPrefix(prefix)
        guard isInside else {
            // Never execute binaries outside the extension folder.
            return extRoot.appending(path: "__denied__").path()
        }
        return candPath
    }
