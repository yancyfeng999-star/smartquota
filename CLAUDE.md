...
SmartQuota is a macOS menu bar application that monitors AI coding assistant usage quotas (Claude, Codex, Gemini, GitHub Copilot, Antigravity, Z.ai, AWS Bedrock, Amp Code, Kimi, OpenCode Go, Oh My Pi, Grok). It probes CLI tools and APIs to fetch quota information and displays it in a menu bar interface with system notifications for status changes.
...
tuist build SmartQuota -C Release
```
...
All settings are persisted in a single JSON file (`~/.smartquota/settings.json`) via `JSONSettingsRepository`.
...
5. **Registration** → Add to `SmartQuotaApp.init()` providers array
...
- **File** (for users): Persistent logs at `~/Library/Logs/SmartQuota/SmartQuota.log`
...
- **Location**: `~/Library/Logs/SmartQuota/SmartQuota.log`
- **Rotation**: Rotates to `SmartQuota.old.log` at 5MB
...
open ~/Library/Logs/SmartQuota/

# Tail the log
tail -f ~/Library/Logs/SmartQuota/SmartQuota.log
```

**OSLog (for developers):**
```bash
# Console.app filter
subsystem:com.smartquota.app
...
log show --predicate 'subsystem == "com.smartquota.app" AND messageType == error' --last 1h

# Live stream (for debugging)
log stream --predicate 'subsystem == "com.smartquota.app"' --info --debug
```
...
grep -i "probe" ~/Library/Logs/SmartQuota/SmartQuota.log

# OSLog - probe-specific logs
log show --predicate 'subsystem == "com.smartquota.app" AND category == "probes"' --info --debug --last 1h
```
