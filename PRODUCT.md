...
| **Mac 版本** | **0.3.29**（build 32，待发布） |
| **Windows 版本** | **0.5.0**（源码；Setup 发布包见 Release） |
| **状态** | **v0.3.29 源码候选；最后已发布 v0.3.28；通用能力与公开治理持续完善**（2026-08） |
...
6. **更新可控**：仅用户点击「检查更新」时下载；优先 `.pkg` 本地解包 + 退出后覆盖（无确认框、**不索要管理员密码**）。目录不可写则设置页报错。默认构建不启用 Sparkle，也不后台强制更新。
7. **开源可复核**：仓库自有代码、文档和脚本采用 Apache-2.0；第三方依赖、资源、商标和服务边界单独声明。
...
- 默认后台轮询或强制更新（用户点击后的 pkg 一键安装除外）；当前源码保留的 `ENABLE_SPARKLE` 条件路径不得在未完成单独审查前启用
...
| **D 可日常使用基线** | ✅ | v0.3.29 源码候选；最后已发布 v0.3.28；基础文档和持续小版本热修；通用能力与公开治理继续补齐 |
